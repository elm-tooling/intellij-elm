...
class ElmFormatOnFileSaveListener : FileDocumentManagerListener {
    override fun beforeDocumentSaving(document: Document) {
        ...
        for (project in ProjectLocator.getInstance().getProjectsForFile(vFile).filterNotNull()) {
            if (!project.elmSettings.toolchain.isElmFormatOnSaveEnabled) continue
            val elmVersion = ElmFormatCLI.getElmVersion(project, vFile) ?: continue
            val elmFormat = project.elmToolchain.elmFormatCLI ?: continue
